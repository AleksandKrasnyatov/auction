...
class RequestTest extends TestCase
{
    public function testSuccess(): void
    {
        $user = (new UserBuilder())
            ->withEmail($old = new Email('old@mail.test'))
            ->active()
            ->build();
        ...
        $user->requestEmailChanging($token, $now, $new = new Email('new@mail.test'));
    ...
    public function testAlready(): void
    {
        ...
        $user->requestEmailChanging($token, $now, $email = new Email('new@mail.test'));
    ...
    public function testExpired(): void
    {
        ...
        $user->requestEmailChanging($token, $now, new Email('new@mail.test'));

        $newDate = $now->modify('+2 hours');
        $newToken = $this->createToken($newDate->modify('+1 hour'));
        $user->requestEmailChanging($newToken, $newDate, $new = new Email('new-new@mail.test'));
